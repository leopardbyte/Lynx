...
import net.minecraft.entity.passive.EntityChicken; // For a chicken
import net.minecraft.entity.passive.EntityCow; // For a cow
import net.minecraft.entity.passive.EntityPig; // For a pig
import net.minecraft.entity.monster.EntitySlime; // For a slime
import net.minecraft.entity.monster.EntityWitch; // For a witch
import net.minecraft.entity.passive.EntitySheep; // For a sheep
import net.minecraft.entity.passive.EntityRabbit; // For a rabbit
import net.minecraft.entity.monster.EntityIronGolem; // For an iron golem
import net.minecraft.entity.monster.EntityMagmaCube; // For a magma cube
...
/*   TODO:
    ...
    -cant walk backwards anymore need to fix... ASAP!! fixed btw u stupid 1 line removale .._..

    -add failsafes when no target is found (fix antistuck lag-maybe threading-)
...
public class EndermanFinder {
    ...
    private boolean setting1State = false;
    private boolean setting2State = false;
    private boolean setting3State = false;
    ...
    public boolean getSetting1State() {
        return setting1State;
    }

    public void setSetting1State(boolean state) {
        this.setting1State = state;
    }

    public boolean getSetting2State() {
        return setting2State;
    }

    public void setSetting2State(boolean state) {
        this.setting2State = state;
    }

    public boolean getSetting3State() {
        return setting3State;
    }

    public void setSetting3State(boolean state) {
        this.setting3State = state;
    }
    ...
    //private AntiStuck antiStuck = new AntiStuck();
    ...
    public List<String> loadCustomEntityNames1() {
        List<String> customNames = new ArrayList<>(DEFAULT_ENTITY_NAMES);
        if (CUSTOM_ENTITY_FILE.exists()) {
            try (BufferedReader reader = new BufferedReader(new FileReader(CUSTOM_ENTITY_FILE))) {
                String line;
                while ((line = reader.readLine()) != null) {
                    customNames.add(line.trim().toLowerCase());
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    "goblin", "ice walker", "crystal sentry", "firemage", "treasuer hunter", "primal fear"
    ));
    ...
    public void setDrawPathEnabled() {
        this.drawPathEnabled = !this.drawPathEnabled; // Toggle the state
        Minecraft.getMinecraft().thePlayer.addChatMessage(
            new ChatComponentText("§6Path drawing toggled: " + (this.drawPathEnabled ? "Enabled" : "Disabled"))
        );
    }
    ...
    private static final Map<String, List<Float>> ENTITY_HP_RANGES = new HashMap<String, List<Float>>() {{
        put("enderman", Arrays.asList(1f, 180000000f)); // Example range for Enderman
        put("zombie", Arrays.asList(1f, 50000000f));
        put("spider", Arrays.asList(1f, 40000000f));
        put("wolf", Arrays.asList(1f, 20000000f));
        put("blaze", Arrays.asList(1f, 100000000f));
        put("chicken", Arrays.asList(1f, 100000000f));
        put("cow", Arrays.asList(1f, 100000000f));
        put("pig", Arrays.asList(1f, 100000000f));
        put("slime", Arrays.asList(1f, 100000000f));
        put("witch", Arrays.asList(1f, 100000000f));
        put("sheep", Arrays.asList(1f, 100000000f));
        put("rabbit", Arrays.asList(1f, 100000000f));
        put("irongolem", Arrays.asList(1f, 100000000f));
        put("magmacube", Arrays.asList(1f, 100000000f));
        put("mooshroom", Arrays.asList(1f, 2000000f));
        put("skeleton", Arrays.asList(1f, 20000000f));
        put("endermite", Arrays.asList(1f, 2000000f));
        put("creeper", Arrays.asList(1f, 3000000f));
        put("armorstand", Arrays.asList(1f, 3000000f));
    ...
    public void init(FMLInitializationEvent event) {
        ...
        ClientCommandHandler.instance.registerCommand(new CommandBase() {
            ...
            public String getCommandUsage(ICommandSender sender) {
                return "§6§l/lynx walk | /lynx hp <add/remove/list> <value> | /lynx target <mob> | /lynx block add";
            }
    ...
    public void toggleKeyAndClickActions() {
        keyAndClickActionsEnabled = !keyAndClickActionsEnabled;
        String status = keyAndClickActionsEnabled ? "§aenabled" : "§cdisabled";
        Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("§6§l[Lynx] §bAuto Heal " + status));
        if (!keyAndClickActionsEnabled) {
            keyAndClickActions.stopActions();
            actionsRunning = false;
        }
    ...
    private void disableModFunctionality() {
        ...
        public void onRenderWorldLast(RenderWorldLastEvent event) {
            Minecraft mc = Minecraft.getMinecraft();
            if (mc.thePlayer == null || mc.theWorld == null || !drawPathEnabled)
                return; // Skip if path drawing is disabled
            ...
            if (currentPath.isEmpty()) {
                ...
                if (nextTarget != null) {
                    ...
                    if (!nextPath.isEmpty() && drawPathEnabled) {
        ...
        private void handleFailedPathfinding(EntityLivingBase target) {
            ...
            if (attempts >= MAX_FAILED_ATTEMPTS) {
                ...
                failedPathAttempts.remove(target);
            }
        }
        ...
        private void antiStuck(Minecraft mc) {
            if (isUnstucking) {
                return; // Prevent multiple unstucking actions simultaneously
            }
            ...
            // If targeting Enderman, reset stuck detection
            if (isTargetingEnderman && currentTime - lastMoveTime <= 2000) {
                lastX = currentX;
                lastZ = currentZ;
            ...
            if (Math.abs(currentX - lastX) < 0.1 && Math.abs(currentZ - lastZ) < 0.1 && currentTime - lastMoveTime > 2000) {
                isUnstucking = true;
                lastMoveTime = currentTime;
        
                // Perform unstuck actions
                new Thread(() -> {
                    try {
                        KeyBinding.setKeyBindState(mc.gameSettings.keyBindJump.getKeyCode(), true);
                        KeyBinding.setKeyBindState(mc.gameSettings.keyBindBack.getKeyCode(), true);
        
                        // Randomly choose left or right key
                        boolean goLeft = Math.random() > 0.5;
                        KeyBinding.setKeyBindState(goLeft ? mc.gameSettings.keyBindLeft.getKeyCode() : mc.gameSettings.keyBindRight.getKeyCode(), true);
        
                        Thread.sleep(1500); // Perform actions for 1.5 seconds
    ...
    public boolean isTargetEntityType(EntityLivingBase entity) {
        ...
        switch (targetEntityType) {
            ...
            case "armorstand":
                // Check if the entity is an armor stand and its name contains the player's name
                if (entity instanceof EntityArmorStand) {
                    return entity.getCustomNameTag() != null && entity.getCustomNameTag().contains(playerName);
                }
                return false;
            case "endermite":
    ...
    // Check if the block is a slab, stair, carpet, rail, tall grass, or specified crops
    ...
    boolean isRail = block instanceof BlockRailBase;
    boolean isTallGrass = block instanceof BlockTallGrass;
    boolean isWalkableCrop = block == Blocks.nether_wart || block == Blocks.wheat ||
                             block == Blocks.carrots || block == Blocks.potatoes;

    // Check if the block below is solid, or it's a slab/stair, rail, carpet, tall grass, or specified crop
    return (mc.theWorld.isAirBlock(pos) || isCarpet || isRail || isTallGrass || isWalkableCrop) && 
           mc.theWorld.isAirBlock(pos.up()) &&
           (isSlabOrStair || blockBelow.isBlockNormalCube() || blockBelow.isOpaqueCube() || isCarpet || isRail || isTallGrass || isWalkableCrop);
}
    ...
    private void drawPath(List<BlockPos> path, float partialTicks, float r, float g, float b, float a) {
        if (path.isEmpty() || !drawPathEnabled) return;
